"""
GCS Upload of FCC Fixed Broadband Coverage Data (raw, extracted csv and processed parquet files).
==============================================================
Uploads:
    * raw zip files to GCS under:       gs://{BUCKET}/network_idx/raw/fcc/broadband_coverage/
    * extracted csv files to GCS under:  gs://{BUCKET}/network_idx/extracted/fcc/broadband_coverage/
    * processed parquet files to GCS under: gs://{BUCKET}/network_idx/processed/fcc/broadband_coverage/
...
Usage:
    ...
    python -m network_idx.transfer.fcc_fixed_summary_gcs --stage processed --all

    # Upload raw zips for selected states
    python -m network_idx.transfer.fcc_fixed_summary_gcs --stage raw --states AK AL IL IN

    # Force re-upload
    python -m network_idx.transfer.fcc_fixed_summary_gcs --stage extracted --states AK --overwrite
"""
...
from network_idx.config import (
    ...
    GCS_PREFIX_RAW_FCC_BROADBAND_COVERAGE,
    GCS_PREFIX_EXTRACTED_FCC_BROADBAND_COVERAGE,
    GCS_PREFIX_PROCESSED_FCC_BROADBAND_COVERAGE,
    UPLOAD_OVERWRITE,
    UPLOAD_CHUNK_MB
)
...
STAGE_CONFIG = {
    "raw": (RAW_DIR_FCC_BROADBAND_COVERAGE, "bdc_*.zip", GCS_PREFIX_RAW_FCC_BROADBAND_COVERAGE),
    "extracted": (EXTRACTED_DIR_FCC_BROADBAND_COVERAGE, "**/*.csv", GCS_PREFIX_EXTRACTED_FCC_BROADBAND_COVERAGE),
    "processed": (PROCESSED_DIR_FCC_BROADBAND_COVERAGE, "fcc_fixed_coverage_*.parquet", GCS_PREFIX_PROCESSED_FCC_BROADBAND_COVERAGE),
}
...
def parse_usps_from_processed_filename(filename: str) -> str | None:
    ...
    '''
    match = re.match(r"fcc_fixed_coverage_([A-Z]{2})_\d{2}\.parquet", filename)
    return match.group(1) if match else None
