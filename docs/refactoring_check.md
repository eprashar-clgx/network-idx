...
| 16 | `features/parcel_features.sql` | 2026-09-24 | `teu_features.parcel_features` | 154,563,179 | ✅ | VM. **Rebuilt post-F7** to pick up corrected fiber distances. 17 cols = 3 keys + 13 model features. Rows = parcel master (no join fan-out/loss). `dist_to_nearest_fiber_miles` null = 5,920,445 (3.8%, matches step-10 exactly); `nearest_fiber_id` 0 numeric-leak. Other nulls unremarkable: cable/housing 144,899 (0.09%), pop_ch_avg 1,523,624 (0.99%), hotspot 51,066,286 (33%, untouched by this fix). |
